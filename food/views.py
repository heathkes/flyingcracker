...
def recipe_list(request, recipe_type=""):
    ...
    if (agent and agent.find('iPhone') != -1) or request.GET.has_key('iphone'):
        return render_to_response('food/recipe_detail_small.html', {'recipe_list' : all_recipes,
                                                                    'foodstuff_list': all_foodstuff,
                                                                    'recipe_type': recipe_type,})
    else:
        all_links = Link.objects.all()
        return render_to_response('food/recipe_list.html', {'all_recipes' : all_recipes,
                                                            'all_links' : all_links,
                                                            'recipe_type': recipe_type,})
...
def recipe_detail(request, slug, recipe_type=""):
    ...
    if (agent and agent.find('iPhone') != -1) or request.GET.has_key('iphone'):
        ...
        all_links = Link.objects.all()
        return render_to_response('food/recipe_detail.html', {'recipe_type': recipe_type,
                                                              'recipe' : r,
                                                              'ingredients' : ingredient_list,
                                                              'all_links' : all_links,})
...
def foodstuff_list(request, recipe_type=""):
    ...
    agent = request.META.get('HTTP_USER_AGENT')
    if (agent and agent.find('iPhone') != -1) or request.GET.has_key('iphone'):
        return render_to_response('food/foodstuff_detail_small.html', {'recipe_list' : all_recipes,
                                                                       'foodstuff_list': all_foodstuff,
                                                                       'recipe_type': recipe_type,})
    else:
        all_links = Link.objects.all()
        return render_to_response('food/foodstuff_list.html', {'all_foodstuff' : all_foodstuff,
                                                               'all_links' : all_links,
                                                               'recipe_type': recipe_type,})
...
def foodstuff_detail(request, slug, recipe_type=""):
    ...
    if (agent and agent.find('iPhone') != -1) or request.GET.has_key('iphone'):
        ...
        all_links = Link.objects.all()
        return render_to_response('food/foodstuff_detail.html', {'foodstuff' : f,
                                                                 'ingredients' : ingredient_list,
                                                                 'all_links' : all_links,
                                                                 'recipe_type': recipe_type,})
